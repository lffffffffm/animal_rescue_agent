...
def collect_evidence(state: AgentState) -> AgentState:
    ...
    base_query = clean_text(state.get("rewrite_query") or state.get("normalized_query") or state.get("query"))
    ...
    species = clean_text(vf.get("species"))
    urgency_level = state.get("urgency")
    ...
    for injury in injuries:
        if isinstance(injury, dict):
            part = clean_text(injury.get("part"))
            itype = clean_text(injury.get("type"))
            if part or itype:
                injury_hints.append(f"{part} {itype}".strip())

    injury_hint = ", ".join(injury_hints)
    red_flag_hint = ", ".join([clean_text(x) for x in red_flags if clean_text(x)])
    ...
    vision_hint_parts = []
    if species:
        vision_hint_parts.append(f"animal={species}")
    if urgency_level:
    ...
    if use_map:
        try:
            location = clean_text(mutable_state.get("location"))
            if not location:
